...
static const char* global_prov_uri = AZIOT_CONF_PROVISION_ENDPOINT;
static const char* id_scope = AZIOT_CONF_PROVISION_ID_SCOPE;

#if (AZIOT_CONF_HSM_TYPE == HSM_TYPE_SYMM_KEY)
static const char* registration_id = AZIOT_CONF_PROVISION_REGISTRATION_ID;
static const char* symmetric_key = AZIOT_CONF_PROVISION_SYMMETRIC_KEY;
#endif
#else
// MQTT protocol
#include "iothubtransportmqtt.h"
...
#elif (AZIOT_CONF_HSM_TYPE == HSM_TYPE_X509)
...
#else
        if ((device_ll_handle = IoTHubDeviceClient_LL_CreateFromConnectionString(AZIOT_CONF_DEVICE_CONNECTION_STRING, iothub_transport) ) == NULL)
        {
            LogInfo("failed create IoTHub client from connection string %s!\r\n", AZIOT_CONF_DEVICE_CONNECTION_STRING);
        }
#endif /* USE_PROV_MODULE_FULL */
        else
        {
